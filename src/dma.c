...
Port port_from_index(uint32_t index) {
    switch (index) {
        case 0: return PORT_MDEC_IN;
        case 1: return PORT_MDEC_OUT;
        case 2: return PORT_GPU;
        case 3: return PORT_CDROM;
        case 4: return PORT_SPU;
        case 5: return PORT_PIO;
        case 6: return PORT_OTC;
        default:
...
uint32_t dma_interrupt(const Dma *dma) {
    return (uint32_t)dma->irq_dummy
         | ((uint32_t)dma->force_irq         << 15)
         | ((uint32_t)dma->channel_irq_en    << 16)
         | ((uint32_t)dma->irq_en            << 23)
         | ((uint32_t)dma->channel_irq_flags << 24)
         | ((uint32_t)dma_irq(dma)           << 31);
}
