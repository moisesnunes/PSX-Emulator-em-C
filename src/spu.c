...
static void decode_adpcm_block(const uint8_t *block, int16_t decoded[28],
    ...
    for (int i = 0; i < 28; i++) {
        ...
        int32_t shifted = raw << (12 - shift);

        int32_t old_   = (int32_t)*old_sample;
        int32_t older_ = (int32_t)*older_sample;
        int32_t filtered;
        switch (filter) {
            case 0: filtered = shifted;                                         break;
            case 1: filtered = shifted + (60 * old_ + 32) / 64;                break;
            case 2: filtered = shifted + (115 * old_ - 52 * older_ + 32) / 64; break;
            case 3: filtered = shifted + (98  * old_ - 55 * older_ + 32) / 64; break;
            case 4: filtered = shifted + (122 * old_ - 60 * older_ + 32) / 64; break;
...
static void adsr_clock(AdsrEnvelope *e, EnvDirection dir, ChangeRate rate,
    ...
    if (dir == DIR_INCREASING && rate == RATE_EXPONENTIAL && e->level > 0x6000)
        decrement >>= 2;
    if (e->counter < decrement) e->counter = 0;
    else                        e->counter -= decrement;
    if (e->counter == 0) {
        e->counter = ENVELOPE_COUNTER_MAX;
...
static void voice_decode_next_block(Voice *v, const uint8_t *sound_ram) {
    ...
    if (loop_start) v->repeat_address = v->current_address;
...
static void voice_store(Voice *v, uint32_t offset, uint16_t val) {
    switch (offset) {
        case 0x00:
            ...
            if (v->enable_sweep_l) v->sweep_l = val;
            else v->volume_l = (int16_t)(val << 1);
            break;
        case 0x02:
            v->enable_sweep_r = (val & 0x8000) != 0;
            if (v->enable_sweep_r) v->sweep_r = val;
            else v->volume_r = (int16_t)(val << 1);
            break;
        case 0x04: v->sample_rate    = val; break;
        case 0x06: v->start_address  = (uint32_t)val << 3; break;
        case 0x08: v->adsr1          = val; break;
        case 0x0A: v->adsr2          = val; break;
        case 0x0E: v->repeat_address = (uint32_t)val << 3; break;
        default: break;
    }
}
...
static int16_t apply_comb_filter(Spu *spu) {
    ...
    return apply_volume(c1, spu->vcomb1) + apply_volume(c2, spu->vcomb2)
         + apply_volume(c3, spu->vcomb3) + apply_volume(c4, spu->vcomb4);
}
...
void spu_destroy(Spu *spu) {
    if (spu->device) SDL_CloseAudioDevice(spu->device);
}

/* ---- Load / Store ---- */
uint16_t spu_load(const Spu *spu, uint32_t abs_addr, uint32_t offset) {
    (void)spu; (void)abs_addr;
    if (offset == 0x01AC) return 0x0004;
    return 0;
}
...
void spu_store(Spu *spu, uint32_t abs_addr, uint32_t offset, uint16_t val) {
    ...
    switch (offset) {
        ...
        case 0x0188:
            for (int i = 0; i < 15; i++)
                if (val & (1 << i)) voice_key_on(&spu->voices[i], spu->sound_ram);
            break;
        case 0x018A:
            for (int i = 0; i < 9; i++)
                if (val & (1 << i)) voice_key_on(&spu->voices[15 + i], spu->sound_ram);
            break;
        case 0x018C:
            for (int i = 0; i < 15; i++)
                if (val & (1 << i)) voice_key_off(&spu->voices[i]);
            break;
        case 0x018E:
            for (int i = 0; i < 9; i++)
                if (val & (1 << i)) voice_key_off(&spu->voices[15 + i]);
            break;
        case 0x0198:
            for (int i = 0; i < 15; i++)
        ...
        case 0x019A:
            ...
            break;
        case 0x0180:
            if (!(val & 0x8000)) spu->main_volume_l = (int16_t)(val << 1);
            break;
        case 0x0182:
            if (!(val & 0x8000)) spu->main_volume_r = (int16_t)(val << 1);
            break;
        case 0x0184: spu->reverb_output_volume_l = (int16_t)val; break;
        case 0x0186: spu->reverb_output_volume_r = (int16_t)val; break;
        case 0x01A2:
            spu->reverb_start_address = (uint32_t)val << 3;
            spu->reverb_write_address = spu->reverb_start_address;
            break;
        case 0x01FC: spu->reverb_input_volume_l = (int16_t)val; break;
        case 0x01FE: spu->reverb_input_volume_r = (int16_t)val; break;
        case 0x01D4: spu->mlsame  = (uint32_t)val << 3; break;
        case 0x01E0: spu->dlsame  = (uint32_t)val << 3; break;
        case 0x01D6: spu->mrsame  = (uint32_t)val << 3; break;
        case 0x01E2: spu->drsame  = (uint32_t)val << 3; break;
        case 0x01E6: spu->mrdiff  = (uint32_t)val << 3; break;
        case 0x01F0: spu->dldiff  = (uint32_t)val << 3; break;
        case 0x01E4: spu->mldiff  = (uint32_t)val << 3; break;
        case 0x01F2: spu->drdiff  = (uint32_t)val << 3; break;
        case 0x01C4: spu->viir    = (int16_t)val; break;
        case 0x01CE: spu->vwall   = (int16_t)val; break;
        case 0x01C6: spu->vcomb1  = (int16_t)val; break;
        case 0x01C8: spu->vcomb2  = (int16_t)val; break;
        case 0x01CA: spu->vcomb3  = (int16_t)val; break;
        case 0x01CC: spu->vcomb4  = (int16_t)val; break;
        case 0x01D8: spu->mlcomb1 = (uint32_t)val << 3; break;
        case 0x01DA: spu->mrcomb1 = (uint32_t)val << 3; break;
        case 0x01DC: spu->mlcomb2 = (uint32_t)val << 3; break;
        case 0x01DE: spu->mrcomb2 = (uint32_t)val << 3; break;
        case 0x01E8: spu->mlcomb3 = (uint32_t)val << 3; break;
        case 0x01EA: spu->mrcomb3 = (uint32_t)val << 3; break;
        case 0x01EC: spu->mlcomb4 = (uint32_t)val << 3; break;
        case 0x01EE: spu->mrcomb4 = (uint32_t)val << 3; break;
        case 0x01C0: spu->dapf1   = (uint32_t)val << 3; break;
        case 0x01C2: spu->dapf2   = (uint32_t)val << 3; break;
        case 0x01D0: spu->vapf1   = (int16_t)val; break;
        case 0x01D2: spu->vapf2   = (int16_t)val; break;
        case 0x01F4: spu->mlapf1  = (uint32_t)val << 3; break;
        case 0x01F6: spu->mrapf1  = (uint32_t)val << 3; break;
        case 0x01F8: spu->mlapf2  = (uint32_t)val << 3; break;
        case 0x01FA: spu->mrapf2  = (uint32_t)val << 3; break;
        default: break;
    }
}
...
void spu_clock(Spu *spu) {
    ...
    if (spu->reverb_left) fir_push(&spu->far_input_l, apf2_out);
    else                  fir_push(&spu->far_input_r, apf2_out);
