...
#define CTRL_RESET      (1u << 4)  /* soft-reset */
...
static const struct { SDL_Scancode sc; uint16_t mask; } KEY_MAP[] = {
...
/* Prepare the RX buffer for a digital-pad transaction. */
static void prepare_pad_response(Sio *sio)
{
    uint8_t lo = (uint8_t)(sio->pad.buttons & 0xFF);
    uint8_t hi = (uint8_t)(sio->pad.buttons >> 8);
    /* Full 5-byte digital pad response (host sends 5 bytes). */
    sio->rx_buf[0] = 0xFF;           /* reply to address byte 0x01 */
    sio->rx_buf[1] = PAD_ID_DIGITAL; /* reply to 0x42: device id   */
    sio->rx_buf[2] = 0x5A;           /* reply to 0x00: ACK         */
    sio->rx_buf[3] = lo;             /* reply to 0x00: buttons lo  */
    sio->rx_buf[4] = hi;             /* reply to 0x00: buttons hi  */
    sio->rx_len = 5;
    sio->rx_pos = 0;
}
...
static uint32_t build_stat(const Sio *sio)
{
    ...
    if (sio->rx_pos < sio->rx_len)
        s |= STAT_RX_AVAIL;
    /* /ACK is asserted (bit=0) while a device is responding */
    if (!sio->selected || sio->rx_pos >= sio->rx_len)
...
uint8_t sio_load8(Sio *sio, uint32_t off)
{
    ...
    case 0x00: { /* RX_DATA */
        ...
        LOG(LOG_SIO, "SIO RX[%u] = 0x%02X", sio->rx_pos - 1, v);
        return v;
    }
    default:
...
void sio_store8(Sio *sio, uint32_t off, uint8_t val, Irq *irq)
{
    (void)irq;
    if (off == 0x00) {
        /* TX_DATA — host sends a byte; we produce the reply byte */
...
{
    switch (off) {
    case 0x00: return (uint16_t)sio_load8(sio, 0x00);
    case 0x04: return (uint16_t)(sio->stat & 0xFFFF);
    case 0x06: return (uint16_t)(sio->stat >> 16);
    case 0x08: return sio->mode;
    case 0x0A: return sio->ctrl;
    case 0x0E: return sio->baud;
    default:   return 0;
    }
}
...
void sio_store16(Sio *sio, uint32_t off, uint16_t val, Irq *irq)
{
    ...
    case 0x0A:
        ...
        if (val & CTRL_RESET) {
            ...
            /* /CS asserted — determine which slot and prepare response */
            bool slot2 = (val & CTRL_SLOT_MASK) != 0;
            sio->selected = true;
            sio->tx_count = 0;
            if (!slot2) {
                prepare_pad_response(sio);
            } else {
                /* Slot 2: no memory card — return 0xFF, no ACK */
                sio->rx_buf[0] = 0xFF;
                sio->rx_len = 1;
                sio->rx_pos = 0;
            }
