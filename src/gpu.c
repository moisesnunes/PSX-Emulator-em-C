...
static uint16_t texel_fetch(const Gpu *gpu, uint8_t u, uint8_t v) {
    ...
    if (gpu->texture_disable) return 0x7FFF;
...
static void draw_pixel(Gpu *gpu, int32_t x, int32_t y, uint16_t color, bool semi) {
    if (x < (int32_t)gpu->drawing_area_left || x > (int32_t)gpu->drawing_area_right) return;
    if (y < (int32_t)gpu->drawing_area_top  || y > (int32_t)gpu->drawing_area_bottom) return;
    uint32_t ux = (uint32_t)x & 1023u;
    uint32_t uy = (uint32_t)y & 511u;
    if (gpu->preserve_masked_pixels && (gpu->vram[uy * 1024u + ux] & 0x8000u)) return;
    ...
    if (semi) {
        ...
        switch (gpu->semi_transparency) {
            default:
            case 0: or_=(br+fr)/2; og=(bg_+fg_)/2; ob=(bb+fb)/2; break; /* (B+F)/2 */
            case 1: or_=br+fr;     og=bg_+fg_;     ob=bb+fb;     break; /* B+F     */
            case 2: or_=(br>fr)?br-fr:0; og=(bg_>fg_)?bg_-fg_:0; ob=(bb>fb)?bb-fb:0; break; /* B-F */
            case 3: or_=br+fr/4;   og=bg_+fg_/4;   ob=bb+fb/4;   break; /* B+F/4   */
        }
        if (or_ > 31u) or_ = 31u;
        if (og  > 31u) og  = 31u;
        if (ob  > 31u) ob  = 31u;
    ...
    if (gpu->force_set_mask_bit) color |= 0x8000u;
...
static void swap_i32(int32_t *a, int32_t *b) { int32_t t=*a;*a=*b;*b=t; }
static void swap_u8 (uint8_t  *a, uint8_t  *b) { uint8_t  t=*a;*a=*b;*b=t; }
...
static void sort3_y(int32_t px[3], int32_t py[3],
    ...
    if (py[0]>py[1]){ swap_i32(&px[0],&px[1]); swap_i32(&py[0],&py[1]);
                      swap_u8(&pr[0],&pr[1]); swap_u8(&pg[0],&pg[1]); swap_u8(&pb[0],&pb[1]); }
    if (py[1]>py[2]){ swap_i32(&px[1],&px[2]); swap_i32(&py[1],&py[2]);
                      swap_u8(&pr[1],&pr[2]); swap_u8(&pg[1],&pg[2]); swap_u8(&pb[1],&pb[2]); }
    if (py[0]>py[1]){ swap_i32(&px[0],&px[1]); swap_i32(&py[0],&py[1]);
                      swap_u8(&pr[0],&pr[1]); swap_u8(&pg[0],&pg[1]); swap_u8(&pb[0],&pb[1]); }
}
...
static void sort3_y_tex(int32_t px[3], int32_t py[3],
                        ...
                        uint8_t pr[3], uint8_t pg[3], uint8_t pb[3]) {
#define SWAP3T(i,j) do { \
    swap_i32(&px[i],&px[j]); swap_i32(&py[i],&py[j]); \
    swap_u8(&pu[i],&pu[j]);  swap_u8(&pv[i],&pv[j]);  \
    swap_u8(&pr[i],&pr[j]);  swap_u8(&pg[i],&pg[j]);  swap_u8(&pb[i],&pb[j]); \
} while(0)
    if (py[0]>py[1]) SWAP3T(0,1);
    if (py[1]>py[2]) SWAP3T(1,2);
    if (py[0]>py[1]) SWAP3T(0,1);
#undef SWAP3T
}
...
static void fill_triangle(Gpu *gpu,
    ...
    if (py[0] == py[2]) return; /* degenerate */
    ...
    for (int32_t y = py[0]; y <= py[2]; y++) {
        ...
        if (xa > xb) {
            swap_i32(&xa,&xb);
            int32_t t; t=ra;ra=rb;rb=t; t=ga;ga=gb;gb=t; t=ba;ba=bb;bb=t;
        }
        int32_t dx = xb-xa;
        for (int32_t x = xa; x <= xb; x++) {
            uint8_t cr, cg, cb;
            if (dx==0) { cr=(uint8_t)ra; cg=(uint8_t)ga; cb=(uint8_t)ba; }
            else {
...
static void fill_triangle_tex(Gpu *gpu,
    ...
    for (int32_t y = py[0]; y <= py[2]; y++) {
        ...
        if (xa > xb) {
            int32_t t; t=xa;xa=xb;xb=t;
            uint8_t tc; tc=ua;ua=ub;ub=tc; tc=va;va=vb;vb=tc;
            t=ra;ra=rb;rb=t; t=ga;ga=gb;gb=t; t=ba;ba=bb;bb=t;
        }
        int32_t dx = xb - xa;
        for (int32_t x = xa; x <= xb; x++) {
            ...
            if (dx==0) {
                fu=ua; fv=va;
                mcr=(uint8_t)ra; mcg=(uint8_t)ga; mcb=(uint8_t)ba;
            } else {
                int32_t t = ((x-xa)<<16)/dx;
...
void gpu_init(Gpu *gpu, SDL_Window *window) {
    ...
    if (!gpu->vram) { fprintf(stderr, "VRAM alloc failed\n"); exit(1); }
...
uint32_t gpu_status(const Gpu *gpu) {
    ...
    switch (gpu->dma_direction) {
        ...
        case DMA_DIR_CPU_TO_GP0:  dma_req = (r >> 28) & 1; break;
        case DMA_DIR_VRAM_TO_CPU: dma_req = (r >> 27) & 1; break;
        default:                  dma_req = 0;              break;
    }
    return r | (dma_req << 25);
}
...
uint32_t gpu_read(Gpu *gpu) {
    ...
    if (gpu->image_store_cur_x >= gpu->image_store_w) {
        gpu->image_store_cur_x = 0; gpu->image_store_cur_y++;
    }
    ...
    if (gpu->image_store_words_remaining == 0) gpu->gp0_mode = GP0_MODE_COMMAND;
...
void gpu_gp0(Gpu *gpu, uint32_t val) {
    if (gpu->gp0_words_remaining == 0) {
        ...
        uint32_t len; void (*method)(Gpu*);
        switch (op) {
            case 0x00: len=1; method=gp0_nop;                       break;
            case 0x01: len=1; method=gp0_clear_cache;               break;
            case 0x02: len=3; method=gp0_fill_rect;                 break;
            /* mono triangles: 0x20-0x23 */
            case 0x20: case 0x21: case 0x22: case 0x23:
                len=4; method=gp0_triangle_mono_opaque;      break;
            /* textured triangles: 0x24-0x27 */
            case 0x24: case 0x25: case 0x26: case 0x27:
                len=7; method=gp0_triangle_tex_opaque;       break;
            /* mono quads: 0x28-0x2B */
            case 0x28: case 0x29: case 0x2A: case 0x2B:
                len=5; method=gp0_quad_mono_opaque;          break;
            /* textured quads: 0x2C-0x2F */
            case 0x2C: case 0x2D: case 0x2E: case 0x2F:
                len=9; method=gp0_quad_tex_opaque;           break;
            /* shaded triangles: 0x30-0x33 */
            case 0x30: case 0x31: case 0x32: case 0x33:
                len=6; method=gp0_triangle_shaded_opaque;    break;
            /* shaded+textured triangles: 0x34-0x37 */
            case 0x34: case 0x35: case 0x36: case 0x37:
                len=9; method=gp0_triangle_shaded_tex_opaque; break;
            /* shaded quads: 0x38-0x3B */
            case 0x38: case 0x39: case 0x3A: case 0x3B:
                len=8; method=gp0_quad_shaded_opaque;        break;
            ...
            case 0x64: case 0x65: case 0x66: case 0x67:
                len=4; method=gp0_rect_variable_tex_opaque;  break;
            /* 1x1 rectangles: 0x68-0x6B */
            case 0x68: case 0x69: case 0x6A: case 0x6B:
                len=2; method=gp0_rect_1x1_opaque;           break;
            /* 1x1 textured rectangles: 0x6C-0x6F */
            case 0x6C: case 0x6D: case 0x6E: case 0x6F:
                len=3; method=gp0_rect_1x1_tex_opaque;       break;
            /* 8x8 rectangles: 0x70-0x73 */
            case 0x70: case 0x71: case 0x72: case 0x73:
                len=2; method=gp0_rect_8x8_opaque;           break;
            /* 8x8 textured rectangles: 0x74-0x77 */
            case 0x74: case 0x75: case 0x76: case 0x77:
                len=3; method=gp0_rect_8x8_tex_opaque;       break;
            /* 16x16 rectangles: 0x78-0x7B */
            case 0x78: case 0x79: case 0x7A: case 0x7B:
                len=2; method=gp0_rect_16x16_opaque;         break;
            /* 16x16 textured rectangles: 0x7C-0x7F */
            case 0x7C: case 0x7D: case 0x7E: case 0x7F:
                len=3; method=gp0_rect_16x16_tex_opaque;     break;
            /* VRAM transfers */
        ...
        case GP0_MODE_IMAGE_LOAD: {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (gpu->image_load_cur_x >= gpu->image_load_w) {
                    gpu->image_load_cur_x = 0; gpu->image_load_cur_y++;
                }
            }
            if (gpu->gp0_words_remaining == 0) gpu->gp0_mode = GP0_MODE_COMMAND;
            break;
        }
        case GP0_MODE_IMAGE_STORE: break;
    }
}
...
static void gp0_triangle_mono_opaque(Gpu *gpu) {
    ...
    apply_offset(gpu,&x0,&y0); apply_offset(gpu,&x1,&y1); apply_offset(gpu,&x2,&y2);
...
static void gp0_quad_shaded_opaque(Gpu *gpu) {
    int32_t xs[4], ys[4]; uint8_t rs[4], gs[4], bs[4];
    for (int i=0;i<4;i++) {
        uint32_t ci = gpu->gp0_command.buffer[i*2];
        rs[i]=ci&0xFF; gs[i]=(ci>>8)&0xFF; bs[i]=(ci>>16)&0xFF;
...
static void gp0_quad_tex_opaque(Gpu *gpu) {
    ...
    int32_t xs[4], ys[4]; uint8_t us[4], vs[4];
    uint32_t w;

    w=gpu->gp0_command.buffer[1]; xs[0]=(int16_t)(w&0xFFFF); ys[0]=(int16_t)(w>>16);
    w=gpu->gp0_command.buffer[2]; us[0]=w&0xFF; vs[0]=(w>>8)&0xFF;
    uint16_t clut_raw=(uint16_t)(w>>16);
    gpu->clut_x=(clut_raw&0x3F)*16; gpu->clut_y=(clut_raw>>6)&0x1FF;

    w=gpu->gp0_command.buffer[3]; xs[1]=(int16_t)(w&0xFFFF); ys[1]=(int16_t)(w>>16);
    w=gpu->gp0_command.buffer[4]; us[1]=w&0xFF; vs[1]=(w>>8)&0xFF;
    uint16_t page_raw=(uint16_t)(w>>16);
    gpu->page_base_x=page_raw&0x0F; gpu->page_base_y=(page_raw>>4)&1;
    switch((page_raw>>7)&3){
        case 0: gpu->texture_depth=TEXTURE_DEPTH_4BIT;  break;
        case 1: gpu->texture_depth=TEXTURE_DEPTH_8BIT;  break;
        case 2: gpu->texture_depth=TEXTURE_DEPTH_15BIT; break;
    ...
    w=gpu->gp0_command.buffer[5]; xs[2]=(int16_t)(w&0xFFFF); ys[2]=(int16_t)(w>>16);
    w=gpu->gp0_command.buffer[6]; us[2]=w&0xFF; vs[2]=(w>>8)&0xFF;
    w=gpu->gp0_command.buffer[7]; xs[3]=(int16_t)(w&0xFFFF); ys[3]=(int16_t)(w>>16);
    w=gpu->gp0_command.buffer[8]; us[3]=w&0xFF; vs[3]=(w>>8)&0xFF;

    for (int i=0;i<4;i++) apply_offset(gpu,&xs[i],&ys[i]);
    bool blend = ((gpu->gp0_command.buffer[0]>>24)&0x01)==0;
    bool semi  = ((gpu->gp0_command.buffer[0]>>24)&0x02)!=0;
...
static void gp0_triangle_shaded_tex_opaque(Gpu *gpu) {
    ...
    int32_t x0=(int16_t)(gpu->gp0_command.buffer[1]&0xFFFF), y0=(int16_t)(gpu->gp0_command.buffer[1]>>16);
    uint32_t uv0c=gpu->gp0_command.buffer[2]; uint8_t u0=uv0c&0xFF, v0=(uv0c>>8)&0xFF;
    ...
    uint32_t uv1p=gpu->gp0_command.buffer[5]; uint8_t u1=uv1p&0xFF, v1=(uv1p>>8)&0xFF;
    ...
    uint32_t uv2=gpu->gp0_command.buffer[8]; uint8_t u2=uv2&0xFF, v2=(uv2>>8)&0xFF;
...
static void gp0_quad_shaded_tex_opaque(Gpu *gpu) {
    int32_t xs[4], ys[4]; uint8_t us[4], vs[4], rs[4], gs[4], bs[4];
    for (int i=0;i<4;i++) {
        uint32_t ci=gpu->gp0_command.buffer[i*3];
        rs[i]=ci&0xFF; gs[i]=(ci>>8)&0xFF; bs[i]=(ci>>16)&0xFF;
        uint32_t xy=gpu->gp0_command.buffer[i*3+1];
        xs[i]=(int16_t)(xy&0xFFFF); ys[i]=(int16_t)(xy>>16);
        uint32_t uv=gpu->gp0_command.buffer[i*3+2];
        us[i]=uv&0xFF; vs[i]=(uv>>8)&0xFF;
        if (i==0) { uint16_t cr2=(uint16_t)(uv>>16); gpu->clut_x=(cr2&0x3F)*16; gpu->clut_y=(cr2>>6)&0x1FF; }
        if (i==1) { uint16_t pr=(uint16_t)(uv>>16);
                    gpu->page_base_x=pr&0x0F; gpu->page_base_y=(pr>>4)&1;
                    switch((pr>>7)&3){case 0:gpu->texture_depth=TEXTURE_DEPTH_4BIT;break;
                                      case 1:gpu->texture_depth=TEXTURE_DEPTH_8BIT;break;
                                      case 2:gpu->texture_depth=TEXTURE_DEPTH_15BIT;break;} }
        apply_offset(gpu,&xs[i],&ys[i]);
    }
...
static void fill_rect_tex_sw(Gpu *gpu, int32_t rx, int32_t ry, int32_t rw, int32_t rh,
    ...
    for (int32_t dy=0; dy<rh; dy++) {
        ...
        if (gpu->rectangle_texture_y_flip) fv = (uint8_t)(v0-dy);
        for (int32_t dx=0; dx<rw; dx++) {
            uint8_t fu = (uint8_t)(u0+dx);
            if (gpu->rectangle_texture_x_flip) fu = (uint8_t)(u0-dx);
            uint16_t texel = texel_fetch(gpu, fu, fv);
            if (texel == 0) continue;
            bool pix_semi = semi && (texel & 0x8000u);
            uint16_t pixel;
...
static void gp0_image_store(Gpu *gpu) {
    ...
    uint16_t w = res & 0xFFFF, h = res >> 16;
    if (w == 0 || h == 0) return;
    gpu->image_store_x = x; gpu->image_store_y = y;
    gpu->image_store_w = w; gpu->image_store_h = h;
...
void gpu_gp1(Gpu *gpu, uint32_t val) {
    ...
    switch (op) {
        ...
        case 0x02: gpu->interrupt = false;                            break;
        case 0x03: gpu->display_disabled = val & 1;                   break;
        case 0x04:
            switch (val & 3) {
