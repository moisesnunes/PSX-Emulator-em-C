...
uint32_t channel_control(const Channel *ch) {
    return ((uint32_t)ch->direction    << 0)
         | ((uint32_t)ch->step        << 1)
         | ((uint32_t)ch->chop        << 8)
         | ((uint32_t)ch->sync        << 9)
         | ((uint32_t)ch->chop_dma_sz << 16)
         | ((uint32_t)ch->chop_cpu_sz << 20)
         | ((uint32_t)ch->enable      << 24)
         | ((uint32_t)ch->trigger     << 28)
         | ((uint32_t)ch->dummy       << 29);
}
...
void channel_set_control(Channel *ch, uint32_t val) {
    ...
    ch->chop        = (val >> 8) & 1;
    switch ((val >> 9) & 3) {
        case 0: ch->sync = SYNC_MANUAL;      break;
        case 1: ch->sync = SYNC_REQUEST;     break;
        case 2: ch->sync = SYNC_LINKED_LIST; break;
        default:
...
int channel_transfer_size(const Channel *ch, uint32_t *out) {
    ...
    switch (ch->sync) {
        case SYNC_MANUAL:  *out = bs;      return 1;
        case SYNC_REQUEST: *out = bc * bs; return 1;
        default:           return 0;
    }
}
