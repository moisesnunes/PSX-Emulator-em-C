...
static int32_t set_mac(Gte *gte, int n, int64_t v, int sf)
{
    ...
    if (v > limit - 1) gte->cr[31] |= (n == 1 ? F_MAC1_POS : n == 2 ? F_MAC2_POS : F_MAC3_POS);
    if (v < -limit)    gte->cr[31] |= (n == 1 ? F_MAC1_NEG : n == 2 ? F_MAC2_NEG : F_MAC3_NEG);
...
{
    if (v >  (int64_t)0x7FFFFFFF) gte->cr[31] |= F_MAC0_POS;
    if (v < -(int64_t)0x80000000) gte->cr[31] |= F_MAC0_NEG;
...
static int16_t set_ir(Gte *gte, int n, int32_t v, bool lm_flag)
{
    ...
    int32_t hi = 0x7FFF;
    if (v < lo || v > hi) {
        gte->cr[31] |= (n == 0 ? F_IR0_SAT : n == 1 ? F_IR1_SAT : n == 2 ? F_IR2_SAT : F_IR3_SAT);
    }
...
{
    if (v < 0 || v > 0x1000) gte->cr[31] |= F_IR0_SAT;
...
{
    if (v < 0 || v > 255) {
        gte->cr[31] |= (ch == 0 ? F_COLOR_R : ch == 1 ? F_COLOR_G : F_COLOR_B);
    }
...
static void push_sxy(Gte *gte, int32_t sx, int32_t sy)
{
    ...
    if (sx < -0x400 || sx > 0x3FF) gte->cr[31] |= F_SX2_SAT;
    if (sy < -0x400 || sy > 0x3FF) gte->cr[31] |= F_SY2_SAT;
...
static void push_sz(Gte *gte, int32_t v)
{
    ...
    gte->dr[17] = gte->dr[18];
    gte->dr[18] = gte->dr[19];
    if (v < 0) { gte->cr[31] |= F_SZ2_OVF; v = 0; }
    if (v > 0xFFFF) { gte->cr[31] |= F_SZ2_OVF; v = 0xFFFF; }
...
static void push_rgb(Gte *gte, int32_t r, int32_t g, int32_t b)
{
    ...
    gte->dr[22] = (uint32_t)sat_rgb(gte, 0, r)
                | ((uint32_t)sat_rgb(gte, 1, g) <<  8)
                | ((uint32_t)sat_rgb(gte, 2, b) << 16)
                | (code                          << 24);
}
...
static void mat_mul_vec(Gte *gte, MatFn mf,
                        ...
                        int sf, bool lm)
{
    int64_t m1 = ((int64_t)tx << 12)
               + (int64_t)mf(gte,0,0) * vx
               + (int64_t)mf(gte,0,1) * vy
               + (int64_t)mf(gte,0,2) * vz;
    int64_t m2 = ((int64_t)ty << 12)
               + (int64_t)mf(gte,1,0) * vx
               + (int64_t)mf(gte,1,1) * vy
               + (int64_t)mf(gte,1,2) * vz;
    int64_t m3 = ((int64_t)tz << 12)
               + (int64_t)mf(gte,2,0) * vx
               + (int64_t)mf(gte,2,1) * vy
               + (int64_t)mf(gte,2,2) * vz;
...
static void cmd_nclip(Gte *gte, uint32_t cmd)
{
    ...
    int64_t v = (int64_t)SX(0) * ((int32_t)SY(1) - (int32_t)SY(2))
              + (int64_t)SX(1) * ((int32_t)SY(2) - (int32_t)SY(0))
              + (int64_t)SX(2) * ((int32_t)SY(0) - (int32_t)SY(1));
    set_mac0(gte, v);
}
...
static void cmd_avsz3(Gte *gte, uint32_t cmd)
{
    ...
    if (otz < 0)       { gte->cr[31] |= F_SZ2_OVF; otz = 0; }
    if (otz > 0xFFFF)  { gte->cr[31] |= F_SZ2_OVF; otz = 0xFFFF; }
...
static void cmd_mvmva(Gte *gte, uint32_t cmd)
{
    ...
    switch (mx) {
    case 0: mf = rt;  break;
    case 1: mf = mat_lm;  break;
    case 2: mf = mat_lcm; break;
    default:
...
static void cmd_dpct(Gte *gte, uint32_t cmd)
{
    ...
    bool lm = (cmd >> 10) & 1;
    gte->cr[31] = 0;
    for (int i = 0; i < 3; i++) depth_cue(gte, sf, lm);
}
...
void gte_execute(Gte *gte, uint32_t cmd)
{
    ...
    switch (op) {
    case 0x01: cmd_rtps(gte, cmd);  break;
    case 0x06: cmd_nclip(gte, cmd); break;
    case 0x0C: cmd_op(gte, cmd);    break;
    case 0x10: cmd_dpcs(gte, cmd);  break;
    case 0x11: cmd_intpl(gte, cmd); break;
    case 0x12: cmd_mvmva(gte, cmd); break;
    case 0x13: cmd_ncds(gte, cmd);  break;
    case 0x14: cmd_cc(gte, cmd);    break;
    case 0x16: cmd_ncs(gte, cmd);   break;
    case 0x1B: cmd_nccs(gte, cmd);  break;
    case 0x1C: cmd_dcpl(gte, cmd);  break;
    case 0x1E: cmd_nct(gte, cmd);   break;
    case 0x20: cmd_sqr(gte, cmd);   break;
    case 0x28: cmd_sqr(gte, cmd);   break; /* SQR alias */
    case 0x29: cmd_dcpl(gte, cmd);  break; /* DCPL alt */
    case 0x2A: cmd_dpct(gte, cmd);  break;
    case 0x2D: cmd_avsz3(gte, cmd); break;
    case 0x2E: cmd_avsz4(gte, cmd); break;
    case 0x30: cmd_rtpt(gte, cmd);  break;
    case 0x3D: cmd_gpf(gte, cmd);   break;
    case 0x3E: cmd_gpl(gte, cmd);   break;
    case 0x3F: cmd_ncct(gte, cmd);  break;
    case 0x17: cmd_ncdt(gte, cmd);  break;
    default:
